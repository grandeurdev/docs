...
In the **Py SDK**, we only use the realtime channel. A device cannot establish a connection over this channel unless and until its access token is validated. A device access token is provided while initialising the grandeur configurations through `grandeur.init()`.